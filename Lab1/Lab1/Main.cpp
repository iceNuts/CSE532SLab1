...
class Player{
public:
	Player(Play& Object, const string& charName, ifstream& inputFile, thread& inputThread) : 
		_playObject(Object),
		_char_name(charName),
		_inputFile(inputFile),
		_playerThread(inputThread)
	{}
	void read(){
		str_op op;
		string str;
	...
	if(ifs.good()){
		...
		while(!ifs.eof()){
			...
			string charname(tokens [0]);
			 ...
			 thread _t = thread();
			 Player player = Player(ref(play), ref(charstrs[i]), ref(files[i]), ref(_t));
